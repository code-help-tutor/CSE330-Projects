 ...
 * 100 MB (5 pts), 200 MB (5 pts), and 300 MB (5 pts), every 10 seconds for 30 seconds
 */
...
int rss_test_case() 
{
	...
	int count 	= 0;
	int wss_range 	= 0;
	int mem_size 	= 26845456; //26845456*sizeof(int) = 100 MB
	wss_range 	= mem_size;
	array 		= (int *) calloc (wss_range, sizeof(int));

	printf("[NOTE] The RSS value reported by the TESTSCRIPT and the Kernel module should match.\n");
	printf("[NOTE] The WSS value expected by the USERSPACE and the Kernel module should match.\n");
	printf("----------------------------------------------------------------------------------\n");

	printf("[USERSPACE]  [PID-%i] Expected WSS:%ld MB\n", getpid(), (wss_range*sizeof(int)/ (1024*1024)));
...
/* 
 * Test Case: 2 
 * Test Criteria: The WSS result from your kernel module output should be 
 * 300 MB (5 pts), 200 MB (5 pts), and 100 MB (5 pts), every 10 seconds for 30 seconds
 */
...
int wss_test_case() 
{
	...
	int mem_size	=  26845456; //26845456*sizeof(int) = 100 MB
	int mem_size_3gb= 3 * mem_size; //300 MB
	...
	if (array != NULL) 
	{
		...
		while (count>=1)
		{
			...
			printf("[USERSPACE]  [PID-%i] Expected [RSS:%ld MB] [WSS:%ld MB]\n", getpid(), (mem_size_3gb*sizeof(int)/(1024 * 1024)), (range*sizeof(int)/(1024*1024)));
  ...
  * pressure of 0.5 GB (5 pts), 1 GB (5 pts), and 1.5 GB (5 pts) 
  */
